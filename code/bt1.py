...
def cr1(url, id_div, cnt):
    # url = 'https://fbref.com/en/comps/9/2023-2024/stats/2023-2024-Premier-League-Stats'
    print(url,id_div)
    requests_1 = requests.get(url)
    Bsoup_1 = bs(requests_1.content, 'html.parser')
    bang1 = Bsoup_1.find('div', {'id': id_div})
    comment1 = bang1.find_all(string=lambda text: isinstance(text, Comment))
    dulieu1 = bs(comment1[0], 'html.parser').find_all('tr')  # chuyển string về dạng tags
    key1 = dict()
    for i, g in enumerate(dulieu1[1].find_all('th')):
        if i !=0:
            key1[g.get('data-stat')] = []
    for i in range(2, len(dulieu1)):
        # tmp chứa tất cả thông tin của player
        tmp1 = dulieu1[i].find_all('td')
        for j, x in enumerate(tmp1):
            if x.get('data-stat') in key1.keys():
                if x.get('data-stat') == 'nationality':
                    s = x.getText().split(" ")
                    key1[x.get('data-stat')].append(s[0])
                else:
                    key1[x.get('data-stat')].append(x.getText())
    df1 = pd.DataFrame(key1)
    if cnt == 2:
        df1.drop(['gk_games', 'gk_games_starts', 'gk_minutes','minutes_90s'],axis=1,inplace=True)
    df1.to_csv(f'table{cnt}.csv')

def clean_data():
    result = pd.read_csv('bang1.csv')
    for x in range(3,11):
